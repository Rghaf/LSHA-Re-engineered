...
def _parse_csv(file_paths, target_vars, trace_config):
    ...
    **vertical**:
    ...
    try:
        ...
        # ---------------------------------------------------------------
        # Step 5 – Interpolate NaN gaps
        #
        # Interpolation fills sensors that report at a lower rate than the
        # master time axis.  Trailing NaNs that interpolate() cannot fill
        # are cleaned by bfill/ffill.
        # ---------------------------------------------------------------
        interp_method = trace_config.get('interpolate_method', 'linear')
        if interp_method and interp_method.lower() != 'none':
            # Restrict numeric interpolation to numeric columns to avoid
            # FutureWarnings / errors on object/string columns.
            ...
            if len(num_cols) > 0:
                ...
                # Special-case those names so the user can still write
                # ``"interpolate_method": "ffill"`` in the JSON.
                if m in ('ffill', 'pad'):
                    df[num_cols] = df[num_cols].ffill().bfill()
                elif m in ('bfill', 'backfill'):
                    df[num_cols] = df[num_cols].bfill().ffill()
                else:
                    df[num_cols] = df[num_cols].interpolate(method=interp_method).bfill().ffill()
        ...
        print(f"[SUL] CSV parse complete – signals: {list(signals.keys())}")
        return signals
    ...
    for driver_name in drivers:
        ...
        try:
            ...
            if tol is None:
                ...
                # jitter.  This is what keeps ENERGY/GREEN sane when the user
                # has not supplied a per-driver tolerance.
                continue
